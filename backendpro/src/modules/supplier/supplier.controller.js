...
class SupplierController {
  ...
  async updateOrderStatus(req, res) {
    try {
      ...
      await db.query(
        'UPDATE orders SET status = $1, updated_at = NOW() WHERE id = $2 AND supplier_id = $3',
  ...
  // ==========================================
  // CAMPAGNES PUBLICITAIRES (Fournisseur - Auth)
  // ==========================================
  ...
  async createCampaign(req, res) {
    try {
      ...
      const result = await db.query(`
        ...
        (supplier_id, name, headline, description, cta_text, cta_link, media_type, media_url, 
         target_products, start_date, end_date, status, views_count, clicks_count)
  ...
  // ==========================================
  // CAMPAGNES PUBLICITAIRES (PUBLIC - Pas d'auth)
  // ==========================================
  async getPublicCampaigns(req, res) {
    try {
      const { supplier, product } = req.query;
      
      console.log(`[PublicCampaigns] Request: supplier=${supplier}, product=${product}`);
      ...
      if (result.rows.length === 0) {
        console.log(`[PublicCampaigns] No active campaign found`);
        return res.json({
          success: true,
          data: null,
      ...
      console.log(`[PublicCampaigns] Found campaign: ${result.rows[0].name}`);
